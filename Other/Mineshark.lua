...
local w,h = term.getSize()
mineshark.infopos = 0
mineshark.infocon,mineshark.infoline = wilmaapi.stringLineTabel(str,w)
...
local ok, err = pcall(mineshark.mainloop)
http.get = http.oldget
--mineshark = nil
mineshark.headbar.setVisible(false)
mineshark.listwindow.setVisible(false)
term.setBackgroundColor(colors.black)
term.setTextColour(colors.white)
term.clear()
term.setCursorPos(1,1)
term.redirect(mineshark.nativeterm)
if not(ok) then
  printError(err)
end
--print("Hallo")
--term.write("hh")
--print()
--return 0local w,h = term.getSize()
os.loadAPI("/usr/apis/wilmaapi")
os.loadAPI("/usr/apis/clipboard")
if type(mineshark) == "table" then
  http.get = mineshark.htppget
